...
That needed `lease_token` in the `UPDATE ... WHERE` clause, so everything the README claims about at-least-once rests on one predicate rather than on three Redis nodes.
